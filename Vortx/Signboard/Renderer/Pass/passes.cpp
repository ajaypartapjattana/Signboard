...
passes::passes(const rhi::creDevice& device, const ctnr::vltView<rhi::pmvImage>& resourceImages)
	: 
	r_device(device),
	r_images(resourceImages)
{

}

uint32_t passes::createSwapchainRenderPass(const rhi::pmvSwapchain& swapchain) {
	rhi::pcdRenderPassCreate pcd{ r_device };
	...
	pcd.target_attachments(attachments);

	uint32_t _subpass = pcd.issue_subpass(VK_PIPELINE_BIND_POINT_GRAPHICS);
	...
	pcd.refer_colorAttachments(_subpass, colorReferences);
	...
	pcd.target_dependancies(dependancies);
...
uint32_t passes::createRenderPass(const renderPassCreateInfo& info) {
	rhi::pcdRenderPassCreate prcdr{ r_device };

	prcdr.target_attachments(info.attachments);

	uint32_t subpass_idx = prcdr.issue_subpass(VK_PIPELINE_BIND_POINT_GRAPHICS);
	...
	prcdr.refer_colorAttachments(subpass_idx, colorReferences);
...
uint32_t passes::createFramebuffers(const uint32_t renderPassIndex, const framebufferCreateInfo& info) {
	rhi::pcdFramebufferCreate pcd{ r_device, *m_renderPasses[renderPassIndex] };

	pcd.push_attachments(r_images, info.attachmentImageIndices);
