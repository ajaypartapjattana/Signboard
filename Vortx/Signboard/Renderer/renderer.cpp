...
namespace rndr {
	...
	Renderer::Renderer(GLFWwindow* window) {
		uint32_t selectedDevice = 0;
		{
			...
			renderContext.enumeratePhysicalDevices(&physicalDeviceCount, nullptr);

			std::vector<const char*> physicalDeviceNames(physicalDeviceCount);
			renderContext.enumeratePhysicalDevices(&physicalDeviceCount, physicalDeviceNames.data());

			for (uint32_t i = 0; i < physicalDeviceCount; ++i) {
				std::cout << "[" << i << "] : " << physicalDeviceNames[i] << std::endl;
			}
		...
		DeviceInfo info{};
		renderContext.createDevice(window, selectedDevice, &info);

		VkDevice device = info.logicalDevice;
		uint32_t graphicsFamilyIndex = info.assignedQueueFamilies[FAMILY_INDEX_GRAPHICS];
		uint32_t presentFamilyIndex = info.assignedPresentFamily;
		
		presentationState.presentationConfiguration(info.nativeDevice, info.candidateSurface);

		PresentationInfo presentInfo;
		presentationState.createSwapchain(device, graphicsFamilyIndex, presentFamilyIndex, &presentInfo);
	...
	void Renderer::createRenderMethods(VkFormat backbufferImageFormat) {
	...
	VkFramebuffer Renderer::createFramebuffer(VkRenderPass renderPass, sgb::span<const VkImageView> attachments, VkExtent2D extent) {
